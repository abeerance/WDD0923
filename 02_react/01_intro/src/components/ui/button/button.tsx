...
export const Button = ({ label }: ButtonProps) => {
  return <button className="h-10 w-10 rounded-lg text-white bg-teal-700">{label}</button>;
};

interface ButtonReactNodeProps {
  children: ReactNode;
}

export const ButtonReactNode = ({ children }: ButtonReactNodeProps) => {
  return <button className="h-10 w-10 rounded-lg text-white bg-teal-700">{children}</button>;
};
