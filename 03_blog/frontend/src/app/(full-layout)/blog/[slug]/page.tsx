...
export default async function BlogPage({ params }: BlogPageProps) {
	...
	const response = await fetchApi<PaginatedArticlesResponse>(
		`articles?slug=${slug}`,
	);
	// fetch request for the user which created the article
	const userResponse = await fetchApi<PublicUser>(
		`users?id=${response.data?.data[0].user_id}`,
	);
	...
	const { title, lead, content, tags, cover_image, created_at } =
